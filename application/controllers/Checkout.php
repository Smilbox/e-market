	...
	public function addOrder()
	{
		...
		if ($this->input->post('payment_option') === "payment_via_mobile_money") {
			...
			if ($mobileMoneyOption === "MVOLA") {
				$add_data['payment_option'] = 'MVOLA';
			} else if ($mobileMoneyOption === "ORANGE_MONEY") {
				$add_data['payment_option'] = 'ORANGE_MONEY';
			} else if ($mobileMoneyOption === "AIRTEL_MONEY") {
				$add_data['payment_option'] = 'AIRTEL_MONEY';
			}
		}
		...
		if ($mobileMoneyOption === "AIRTEL_MONEY") {
			...
			$fields = array(
				...
				"subscriber" => array(
					"country" => "MG",
					"currency" => "MGA",
					"msisdn" => $airtel_phone_number[0] === "0" ? substr($airtel_phone_number, 1) : $airtel_phone_number
				),
				"transaction" => array(
