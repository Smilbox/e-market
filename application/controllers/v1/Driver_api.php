        ...
        if($tokenres){
            ...
            if($order_id){
                ...
                if(!empty($details)){
                    if($this->post('order_status') == 'cancel'){
                    ...
                    }elseif($this->post('order_status') == 'onGoing'){
                        $add_data = array('order_id'=>$order_id,'order_status'=>'ongoing','time'=>date('Y-m-d H:i:s'),'status_created_by'=>'Driver');
                        $this->driver_api_model->addRecord('order_status',$add_data);
                        $detail = $this->driver_api_model->acceptOrder($order_id,$driver_map_id,$user_id, 'ongoing');
                        // adding notification for website
                        $order_detail = $this->driver_api_model->getRecord('order_master','entity_id',$order_id);
                        $notification = array(
                            'order_id' => $order_id,
                        ...
                        $this->driver_api_model->addRecord('user_order_notification',$notification);
                        $this->response(['user_detail'=>$detail,'status'=>1,'message' => 'Order set to ongoing with success'], REST_Controller::HTTP_OK); // OK */
                    }else{
